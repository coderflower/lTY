...
extension DetailViewController {
    ...
    func createTextProvider(dataSource: ArrayDataSource<HomeViewCellViewModel>) -> BasicProvider<HomeViewCellViewModel, UILabel> {
        ...
        let sizeSource = { (_: Int, data: HomeViewCellViewModel, collectionSize: CGSize) -> CGSize in
            CGSize(width: collectionSize.width, height: data.textHeight)
        }
    ...
    func createImageProvider(dataSource: ArrayDataSource<UIImage>) -> BasicProvider<UIImage, UIImageView> {
        let viewSource = ClosureViewSource<UIImage, UIImageView>(viewGenerator: { (_, _) -> UIImageView in
        ...
        let sizeSource = { (_: Int, data: UIImage, collectionSize: CGSize) -> CGSize in
            CGSize(width: collectionSize.width, height: collectionSize.width * data.size.height / data.size.width)
        }
