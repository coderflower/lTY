...
struct Provider {
    ...
    func homeProvider(dataSource: ArrayDataSource<HomeViewCellViewModel>) -> BasicProvider<HomeViewCellViewModel, HomeItemViewCell> {
        let viewSource = ClosureViewSource<HomeViewCellViewModel, HomeItemViewCell>(viewGenerator: { (_, _) -> HomeItemViewCell in
            ...
            view.cornerRadius = 5
            return view
        }, viewUpdater: { (view: HomeItemViewCell, data: HomeViewCellViewModel, _: Int) in
            view.update(data)
        })
        let sizeSource = { (_: Int, data: HomeViewCellViewModel, collectionSize: CGSize) -> CGSize in
            CGSize(width: collectionSize.width, height: data.cellHeight)
        }
    ...
    func profileProvider(dataSource: ArrayDataSource<ProfileModel>) -> BasicProvider<ProfileModel, ProfileViewCell> {
        let viewSource = ClosureViewSource<ProfileModel, ProfileViewCell>(viewUpdater: { (view: ProfileViewCell, data: ProfileModel, _: Int) in
            view.update(data)
        })
        let sizeSource = { (_: Int, _: ProfileModel, collectionSize: CGSize) -> CGSize in
            CGSize(width: collectionSize.width, height: 44)
        }
    ...
    func photoWallProvider(dataSource: ArrayDataSource<PhotoWallViewCellViewModel>) -> BasicProvider<PhotoWallViewCellViewModel, PhotoWallViewCell> {
        let viewSource = ClosureViewSource<PhotoWallViewCellViewModel, PhotoWallViewCell>(viewUpdater: { (view: PhotoWallViewCell, data: PhotoWallViewCellViewModel, _: Int) in
            view.update(data)
        })
        let sizeSource = { (_: Int, _: PhotoWallViewCellViewModel, collectionSize: CGSize) -> CGSize in
            CGSize(width: collectionSize.width, height: collectionSize.width * 0.5)
        }
