...
extension Reactive where Base: UIAlertController {
    ...
    public func show(in vc: UIViewController, buttons: [UIAlertController.AlertButton], textFields: [TextFieldConfiguration?]) -> Single<(Int, [String])> {
        return Single<(Int, [String])>.create(subscribe: { [weak vc] observer in
            ...
            for index in 0 ..< buttons.count {
                let handler = { [unowned alertView] (_: UIAlertAction) -> Void in
    ...
    public func show(in vc: UIViewController, closeTitle: String) -> Single<Void> {
        return show(in: vc, buttons: [.cancel(closeTitle)], textFields: []).map { _ in }
    }
    ...
    public static func show(in vc: UIViewController, title: String?, message: String?, closeTitle: String) -> Completable {
        return show(in: vc, title: title, message: message, buttons: [.cancel(closeTitle)], textFields: []).map { _ in }.asCompletable()
    }
