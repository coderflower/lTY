...
extension String {
    ...
    func calculateSize(font: UIFont, maxWidth: CGFloat = UIScreen.main.bounds.size.width) -> CGSize {
        ...
        var textSize = asNSString.boundingRect(with: size, options: [.usesLineFragmentOrigin], attributes: [NSAttributedString.Key.font: font], context: nil).size
...
extension String {
    ...
    private func validateText(validatedType type: ValidatedType) -> Bool {
        do {
            ...
            let matches = regex.matches(in: self, options: NSRegularExpression.MatchingOptions.reportProgress, range: NSMakeRange(0, count))
    ...
    func phoneNumberhideMid() -> String {
        let startIndex = index("".startIndex, offsetBy: 4)
        let endIndex = index("".startIndex, offsetBy: 7)
        var tmpStr = self
    ...
    mutating func numberHideMidWithOtherChar(form: Int, to: Int, char: String) {
        ...
        let startIndex = index("".startIndex, offsetBy: form)
        let endIndex = index("".startIndex, offsetBy: to)
        replaceSubrange(startIndex ... endIndex, with: star)
    }
}
